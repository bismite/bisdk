...
if [ -e ${INSTALL_PATH}/lib/libbi.a ]; then
  exit
fi
...
# compile
BUILD_DIR=build/$1/bi-core
mkdir -p $BUILD_DIR
for FILE in `find $BI_CORE_DIR/src/* -type f`
do
  NAME=`basename $FILE`
...
cp $BUILD_DIR/libbi.a ${INSTALL_PATH}/lib
cp -R $BI_CORE_DIR/include/* ${INSTALL_PATH}/include
