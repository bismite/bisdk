...
if [ ! -e $FRAMEWORKS_DIR/$SDL2_FRAMEWORK ]; then
  if [ ! -e build/host/$SDL2_DMG ]; then
    curl --progress-bar -S -L -o build/host/$SDL2_DMG https://www.libsdl.org/release/$SDL2_DMG
  fi
...
if [ ! -e $FRAMEWORKS_DIR/$SDL2_IMAGE_FRAMEWORK ]; then
  if [ ! -e build/host/$SDL2_IMAGE_DMG ]; then
    curl --progress-bar -S -L -o build/host/$SDL2_IMAGE_DMG https://www.libsdl.org/projects/SDL_image/release/$SDL2_IMAGE_DMG
  fi
...
if [ ! -e $FRAMEWORKS_DIR/$SDL2_MIXER_FRAMEWORK ]; then
  if [ ! -e build/host/$SDL2_MIXER_DMG ]; then
    curl --progress-bar -S -L -o build/host/$SDL2_MIXER_DMG https://www.libsdl.org/projects/SDL_mixer/release/$SDL2_MIXER_DMG
  fi
